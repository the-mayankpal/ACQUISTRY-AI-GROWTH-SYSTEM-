  ...
  return (
    ...
    `}>
      <div className="max-w-7xl mx-auto px-3 md:px-6">
        ...
        `}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 cursor-pointer z-50 shrink-0">
              ...
              <span className="text-[#101418] text-lg md:text-2xl font-black tracking-tight">Acquisity</span>
            </div>
            ...
            <nav className="hidden lg:flex items-center gap-8" aria-label="Main navigation">
              {NAV_ITEMS.map((item) => (
                <a 
                  key={item.label} 
                  className="text-[15px] font-semibold text-gray-600 hover:text-[#0066ff] transition-colors" 
                  href={item.href}
                >
                  {item.label}
                </a>
        ...
        <div 
          id="mobile-menu"
          className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out bg-white border-x border-b border-gray-200 rounded-b-2xl ${
