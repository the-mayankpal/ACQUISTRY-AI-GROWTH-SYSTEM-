...
export const FAQ: React.FC = () => {
  const [openIndex, setOpenIndex] = useState<number | null>(1); // Defaulting the SDR question open as per image
  ...
  return (
    <section id="faq" className="max-w-3xl mx-auto px-6 lg:px-20 py-24 border-t border-gray-100">
          ...
          return (
            <div key={index} className="border border-gray-200 rounded-xl overflow-hidden transition-all duration-300">
              ...
              >
                ...
                <span className={`material-symbols-outlined transition-transform duration-300 ${isOpen ? 'rotate-45 text-[#0066ff]' : 'text-gray-400'}`}>
                  add
                </span>
              </button>
              <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
                <div className="p-6 pt-0 text-gray-600 leading-relaxed bg-gray-50 text-lg">
                  {item.answer}
                </div>
              </div>
            </div>
